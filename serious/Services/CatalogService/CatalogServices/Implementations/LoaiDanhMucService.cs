...
namespace CatalogService.CatalogServices.Implementations
{
    public class LoaiDanhMucService : ILoaiDanhMucService
    {
        ...
        public async Task<LoaiDanhMucDTO?> GetBySlugAsync(string slug)
        {
            var ldm = await _context.LoaiDanhMucs
                .Include(x => x.DanhMucs)
        ...
        public async Task<LoaiDanhMucDTO> CreateAsync(LoaiDanhMucCreateUpdateDTO dto)
        {
            var entity = new LoaiDanhMuc
            {
                MaLDM = Guid.NewGuid(),
                TenLDM = dto.TenLDM,
                Slug = SlugHelper.GenerateSlug(dto.TenLDM)
            };
        ...
        public async Task<bool> UpdateAsync(Guid id, LoaiDanhMucCreateUpdateDTO dto)
        {
            ...
            entity.Slug = SlugHelper.GenerateSlug(dto.TenLDM);
        ...
        public async Task FixSlugsAsync()
        {
            var loais = await _context.LoaiDanhMucs.ToListAsync();
            foreach (var l in loais)
            {
                if (string.IsNullOrEmpty(l.Slug))
                {
                    l.Slug = SlugHelper.GenerateSlug(l.TenLDM);
                }
            }

            var dms = await _context.DanhMucs.ToListAsync();
            foreach (var d in dms)
            {
                if (string.IsNullOrEmpty(d.Slug))
                {
                    d.Slug = SlugHelper.GenerateSlug(d.TenDM);
                }
            }
